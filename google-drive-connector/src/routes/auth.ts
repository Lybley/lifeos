...
router.get('/google/callback', async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.info(`Retrieved Drive email: ${driveEmail}`);

    await storeTokens(userId, driveEmail, tokens);
    ...
    logger.info(`Successfully connected Google Drive account for user ${userId}`);
      ...
      <html>
        ...
        <body>
          <div class="container">
            ...
            <p>Your Google Drive account has been connected to LifeOS.</p>
            <div class="email">${driveEmail}</div>
            <div class="info">
              <strong>What's happening now:</strong><br>
              We're syncing your Drive files from the last 6 months. This may take a few minutes depending on the volume.
            </div>
      ...
      <html>
        ...
        <body>
          <div class="container">
            ...
            <p>We couldn't connect your Google Drive account. Please try again.</p>
...
router.post('/disconnect', async (req: Request, res: Response) => {
  ...
  try {
    ...
    logger.info(`Disconnected Google Drive for user ${userId}`);
    ...
    logger.error('Failed to disconnect Drive:', error);
    res.status(500).json({
...
router.get('/status', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!tokens) {
      return res.json({
        connected: false,
        message: 'No Google Drive account connected',
      });
    }

    res.json({
      connected: true,
      drive_email: tokens.drive_email,
